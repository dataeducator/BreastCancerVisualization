...
def data_story_page():
    st.header("Inside the Data: Wisconsin Breast Cancer")
    ...
    with st.expander("1. Dataset Overview & Feature Distributions", expanded=True):
        ...
        with col1:
            ...
            fig = px.histogram(
                x=data.data[:, 0],
                color=list(data.target_names[data.target]),  # Key fix: convert to list
                nbins=50,
    ...
    with st.expander("4. Model Performance Comparison", expanded=True):
        ...
        st.plotly_chart(fig, use_container_width=True)
        st.markdown("""
        **Insight:** Both models achieve excellent performance, but SVM shows slightly better results in terms of false negatives (critical for cancer detection), and offers simpler deployment with fewer parameters to tune.
        """)

        # --- Box Plots for Radius Mean by Diagnosis for ANN and SVM ---
        st.subheader("Box Plots: Radius Mean by Diagnosis for ANN and SVM")
        box_df = pd.DataFrame({
            "Radius Mean": X_test[:, 0],
            "Diagnosis": np.where(y_test == 0, "Benign", "Malignant"),
            "SVM Prediction": np.where(svm_pred == 0, "Benign", "Malignant"),
            "ANN Prediction": np.where(ann_pred == 0, "Benign", "Malignant")
        })
        col1, col2 = st.columns(2)
        with col1:
            fig_box_svm = px.box(
                ...
                y="Radius Mean",
                points="all",
                title="SVM: Radius Mean by Diagnosis"
            )
            st.plotly_chart(fig_box_svm, use_container_width=True)
        with col2:
            fig_box_ann = px.box(
                box_df,
                x="Diagnosis",
                y="Radius Mean",
                points="all",
                title="ANN: Radius Mean by Diagnosis"
            )
            st.plotly_chart(fig_box_ann, use_container_width=True)
            st.markdown("""
            **Insight:** Malignant tumors have ~2.5x larger radii than benign tumors.
            """)
