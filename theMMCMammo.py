...
def data_story_page():
    st.header("How Can Algorithms help?")
    ...
    with st.expander("4. Model Performance Comparison", expanded=True):
        ...
        fig.update_layout(
            ...
            yaxis_title='Score',
            yaxis=dict(range=[0.9, 1.0])  # Adjust for better visualization
        )
        st.plotly_chart(fig, use_container_width=True)

        st.markdown("""
        **Insight:** While both models achieve excellent performance, the SVM model shows slightly better
